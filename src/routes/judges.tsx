...
export const Route = createFileRoute("/judges")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Industry leaders judging Compass Crew hackathons and programs.",
      },
    ...
    <>
      ...
      <Section>
        {isLoading ? (
          ...
          <EmptyState
            icon={Gavel}
            title="Judges will be announced before each event."
            description="Follow along for our next hackathon's judging panel."
          />
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {judges.map((j) => (
              <Card key={j.id}>
                <CardContent className="space-y-4 p-6">
                  ...
                  <div className="flex gap-2 pt-1">
                    {safeExternalUrl(j.linkedin_url) && (
                      <a
                        href={safeExternalUrl(j.linkedin_url)}
                        target="_blank"
                    ...
                    {safeExternalUrl(j.twitter_url) && (
                      <a
                        href={safeExternalUrl(j.twitter_url)}
                        target="_blank"
                        rel="noreferrer noopener"
                        aria-label="Twitter"
                        className="text-muted-foreground hover:text-foreground"
