...
function LeaderboardPage() {
  ...
  const hackQ = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("hackathons").select("id, title, tagline, results_published_at, leaderboard_frozen").eq("slug", slug).maybeSingle();
      return data;
    },
  });
  const lbQ = useQuery({
  ...
  const byTrack = useMemo(() => (lbQ.data ? groupByTrack([...lbQ.data]) : new Map<string, LeaderboardEntry[]>()), [lbQ.data]);
    ...
    <>
      ...
      >
        <Button asChild variant="outline">
          <Link to="/hackathons/$slug" params={{ slug }}><ArrowLeft className="mr-2 h-4 w-4" />Hackathon</Link>
        </Button>
      </PageHeader>
      <Section>
          ...
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {e.team_name ?? "Team"}{e.track_name ? ` · ${e.track_name}` : ""}
                {e.judge_count > 0 ? ` · ${e.judge_count} judge${e.judge_count > 1 ? "s" : ""}` : ""}
              </p>
            </div>
            {e.submission.award && (
              <Badge className="hidden sm:inline-flex"><Medal className="mr-1 h-3 w-3" />{e.submission.award}</Badge>
            )}
            <div className="text-right">
              <p className="font-display text-xl font-semibold text-primary">{e.weighted_score.toFixed(2)}</p>
