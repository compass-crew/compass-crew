...
function JudgeHackathonPage() {
  ...
  const hackQ = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("hackathons").select("id, title, tagline").eq("id", hackathonId).maybeSingle();
      return data;
    },
  });
  ...
  const myScoresQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .in("submission_id", subsQ.data.map((s) => s.id))
        .eq("judge_id", user.id)
        .eq("is_final", true);
    ...
    <>
      ...
      >
        <Button asChild variant="outline">
          <Link to="/judge"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link>
        </Button>
      </PageHeader>
      <Section>
        {subsQ.isLoading ? (
          ...
          <EmptyState title="No submitted projects yet" description="Teams still have time to submit. Check back after the deadline." />
        ) : (
                  ...
                  <CardContent className="flex flex-wrap items-center justify-between gap-4 p-6">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                        ...
                        {s.github_url && <a href={s.github_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-foreground"><Github className="h-3.5 w-3.5" /> Repo</a>}
                        {s.live_url && <a href={s.live_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-foreground"><Globe className="h-3.5 w-3.5" /> Demo</a>}
                        {s.video_url && <a href={s.video_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-foreground"><Video className="h-3.5 w-3.5" /> Video</a>}
                        {s.presentation_url && <a href={s.presentation_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-foreground"><FileText className="h-3.5 w-3.5" /> Deck</a>}
                      </div>
                    </div>
                    <Button asChild>
