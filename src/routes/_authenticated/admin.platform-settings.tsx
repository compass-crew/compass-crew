...
export const Route = createFileRoute("/_authenticated/admin/platform-settings")({
  beforeLoad: async (opts) => {
    await requireRole(["super_admin"])(opts);
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Configure branding, navigation, SEO, and analytics for the entire platform. Changes go
          live immediately.
        </p>
      </header>
      ...
      <Tabs defaultValue="general" className="space-y-4">
        ...
        <TabsContent value="seo">
          <SeoForm initial={settings.seo} busy={saving === "seo"} onSave={(d) => save("seo", d)} />
        </TabsContent>
...
function GeneralForm({
  ...
  return (
    <SectionShell
      title="General"
      description="Public platform identity."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
...
function BrandingForm({
  ...
  return (
    <SectionShell
      title="Branding"
      description="Logo, favicon, hero image, and theme colours."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
    >
      ...
      <p className="text-xs text-muted-foreground">
        Colours are stored here and consumed by pages that opt into dynamic theming. Existing design
        tokens continue to control the current UI.
      </p>
      <SaveBar busy={busy} />
    </SectionShell>
  );
...
function HomepageForm({
  ...
  initial?: SettingsMap["homepage"];
  busy: boolean;
  onSave: (d: SettingsMap["homepage"]) => void;
}) {
  const [s, setS] = useSectionState(initial, {
    hero_title: "",
    hero_subtitle: "",
  ...
  return (
    <SectionShell
      title="Homepage"
      description="Hero text, calls to action, and headline statistics."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            value={s.cta_primary_label}
            onChange={(e) => setS({ ...s, cta_primary_label: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>Primary CTA URL</Label>
          <Input
            value={s.cta_primary_url}
            onChange={(e) => setS({ ...s, cta_primary_url: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>Secondary CTA label</Label>
          <Input
            value={s.cta_secondary_label}
            onChange={(e) => setS({ ...s, cta_secondary_label: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>Secondary CTA URL</Label>
          <Input
            value={s.cta_secondary_url}
            onChange={(e) => setS({ ...s, cta_secondary_url: e.target.value })}
          />
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            onClick={() => setS({ ...s, stats: [...s.stats, { label: "", value: "" }] })}
          >
        ...
        {s.stats.length === 0 ? (
          ...
          s.stats.map((stat, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input
                placeholder="Label (e.g. Hackathons)"
                value={stat.label}
                onChange={(e) => {
                  const next = [...s.stats];
              ...
              <Button
                ...
                size="icon"
                variant="ghost"
                onClick={() => setS({ ...s, stats: s.stats.filter((_, j) => j !== i) })}
              >
...
function NavigationForm({
  ...
  initial?: SettingsMap["navigation"];
  busy: boolean;
  onSave: (d: SettingsMap["navigation"]) => void;
}) {
...
function FooterForm({
  ...
  initial?: SettingsMap["footer"];
  busy: boolean;
  onSave: (d: SettingsMap["footer"]) => void;
}) {
  const [s, setS] = useSectionState(initial, { tagline: "", columns: [], copyright: "" });
  return (
    <SectionShell
      title="Footer"
      description="Footer tagline, columns of links, and copyright line."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            onClick={() => setS({ ...s, columns: [...s.columns, { title: "", links: [] }] })}
          >
          ...
          <div key={i} className="rounded-md border border-border p-3 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="icon"
                variant="ghost"
                onClick={() => setS({ ...s, columns: s.columns.filter((_, j) => j !== i) })}
              >
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
...
function ContactForm({
  ...
  initial?: SettingsMap["contact"];
  busy: boolean;
  onSave: (d: SettingsMap["contact"]) => void;
}) {
  const [s, setS] = useSectionState(initial, {
    support_email: "",
    press_email: "",
  ...
  return (
    <SectionShell
      title="Contact"
      description="Official contact details displayed publicly."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="grid gap-2">
          ...
          <Input
            type="email"
            value={s.press_email}
            onChange={(e) => setS({ ...s, press_email: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>Phone</Label>
...
function SocialForm({
  ...
  return (
    <SectionShell
      title="Social"
      description="Public profile URLs displayed in navbar and footer."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
...
function SeoForm({
  ...
  return (
    <SectionShell
      title="SEO"
      description="Default meta tags used across pages unless overridden."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
...
function AnalyticsForm({
  ...
  initial?: SettingsMap["analytics"];
  busy: boolean;
  onSave: (d: SettingsMap["analytics"]) => void;
}) {
  const [s, setS] = useSectionState(initial, {
    ga4_id: "",
    gtm_id: "",
  ...
  return (
    <SectionShell
      title="Analytics"
      description="IDs for analytics providers. Values are safe to expose publicly."
      onSubmit={(e) => {
        e.preventDefault();
        onSave(s);
      }}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            ...
            value={s.ga4_id}
            onChange={(e) => setS({ ...s, ga4_id: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          ...
          <Input
            ...
            value={s.gtm_id}
            onChange={(e) => setS({ ...s, gtm_id: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>Plausible domain</Label>
          <Input
            placeholder="compasscrew.com"
            value={s.plausible_domain}
            onChange={(e) => setS({ ...s, plausible_domain: e.target.value })}
          />
        </div>
        <div className="grid gap-2">
          <Label>PostHog key</Label>
          <Input
            value={s.posthog_key}
            onChange={(e) => setS({ ...s, posthog_key: e.target.value })}
          />
        </div>
      </div>
      <SaveBar busy={busy} />
