...
function MediaPage() {
  ...
  useEffect(() => { void load(prefix); }, [prefix, load]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const walk = async (p: string, depth: number) => {
        if (depth > 2) return;
        const { data } = await supabase.storage.from(CMS_BUCKET).list(p || undefined, { limit: 1000 });
        for (const e of data ?? []) {
  ...
  function pathOf(name: string) { return prefix ? `${prefix}/${name}` : name; }
    ...
    <div className="space-y-6" onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Manage every file across Compass Crew content.</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline">Total: {totalBytes == null ? "…" : bytes(totalBytes)}</Badge>
          <div className="flex overflow-hidden rounded-md border border-border">
            <Button variant={view === "grid" ? "secondary" : "ghost"} size="sm" onClick={() => setView("grid")}><Grid3x3 className="h-4 w-4" /></Button>
            <Button variant={view === "list" ? "secondary" : "ghost"} size="sm" onClick={() => setView("list")}><ListIcon className="h-4 w-4" /></Button>
          </div>
          <input ref={inputRef} type="file" multiple hidden onChange={(e) => handleUpload(e.target.files)} />
          <Button size="sm" onClick={() => inputRef.current?.click()}><Upload className="mr-1.5 h-4 w-4" /> Upload</Button>
        </div>
      </header>
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          <div className="flex flex-wrap items-center gap-1 text-sm">
            <button className="inline-flex items-center gap-1 rounded px-2 py-1 hover:bg-muted" onClick={() => setPrefix("")}>
              <Home className="h-3.5 w-3.5" /> cms-media
            </button>
            {crumbs.map((c, i) => (
              <span key={i} className="flex items-center gap-1">
                <span className="text-muted-foreground">/</span>
                <button className="rounded px-2 py-1 hover:bg-muted" onClick={() => setPrefix(crumbs.slice(0, i + 1).join("/"))}>{c}</button>
              </span>
            ))}
            {prefix && (
              <Button variant="ghost" size="sm" className="ml-2" onClick={() => setPrefix(crumbs.slice(0, -1).join("/"))}>
          ...
          <div className="relative flex-1">
            ...
            <Input placeholder="Filter in this folder…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
        </CardContent>
      </Card>
      ...
      {loading ? (
        ...
        <EmptyState icon={FolderOpen} title="Nothing here yet" description="Drop files anywhere on this page to upload." />
      ) : view === "grid" ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {filtered.map((e) => {
            if (!e.id) {
              return (
                <button key={e.name} onClick={() => setPrefix(pathOf(e.name))} className="flex flex-col items-center gap-2 rounded-xl border border-border/60 bg-card p-4 transition hover:-translate-y-0.5 hover:shadow-elegant">
            ...
            return (
              <div key={e.name} className="group relative overflow-hidden rounded-xl border border-border/60 bg-card">
                <div className="grid aspect-square w-full place-items-center bg-muted/30">
                  {isImage && url ? <img src={url} alt={e.name} loading="lazy" className="h-full w-full object-cover" /> : <Icon className="h-10 w-10 text-muted-foreground" />}
                </div>
                ...
                <div className="absolute right-1.5 top-1.5 flex gap-1 opacity-0 transition group-hover:opacity-100">
                  <Button size="icon" variant="secondary" className="h-7 w-7" onClick={() => copyUrl(e)}><Copy className="h-3.5 w-3.5" /></Button>
          ...
          <CardContent className="p-0">
            <div className="divide-y">
              {filtered.map((e) => {
                if (!e.id) {
                  return (
                    <button key={e.name} onClick={() => setPrefix(pathOf(e.name))} className="flex w-full items-center gap-3 p-3 text-left hover:bg-muted/40">
                ...
                return (
                  <div key={e.name} className={cn("flex items-center gap-3 p-3")}>
                    ...
                    <span className="hidden text-xs text-muted-foreground md:inline">{bytes(e.metadata?.size)}</span>
                    <span className="hidden text-xs text-muted-foreground lg:inline">{e.updated_at ? new Date(e.updated_at).toLocaleDateString() : ""}</span>
                    <Button size="sm" variant="ghost" onClick={() => copyUrl(e)}><Copy className="h-3.5 w-3.5" /></Button>
                    <DeleteButton onConfirm={() => handleDelete(e)} name={e.name} />
                  </div>
                );
              })}
