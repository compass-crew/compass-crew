...
import { ArrowLeft, Github, Globe, Video, FileText, Users, Sparkles, ShieldCheck } from "lucide-react";
...
import {
  getSubmissionForJudge, listTeamMembers, getMyScores, saveDraftScores, finalizeScores, computeWeighted,
} from "@/lib/judging";
...
interface Entry { criterion_id: string; score: number; comment: string; }
...
function ScoringPage() {
  ...
  if (subQ.isLoading) return <Section><Skeleton className="h-96 w-full" /></Section>;
  if (!subQ.data) return <Section><EmptyState title="Submission not found" /></Section>;
  const s = subQ.data;
          ...
          <div className="space-y-6 lg:col-span-2">
            <Card><CardContent className="space-y-4 p-6">
              <div className="flex flex-wrap gap-2">
                ...
                {s.team?.name && <Badge variant="outline"><Users className="mr-1 h-3 w-3" /> {s.team.name}</Badge>}
              </div>
                ...
                {s.tech_stack?.length ? (
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {s.tech_stack.map((t) => <Badge key={t} variant="outline">{t}</Badge>)}
                    </div>
                  </div>
                ) : null}
                {s.ai_models?.length ? (
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">AI models</p>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {s.ai_models.map((t) => <Badge key={t} variant="outline"><Sparkles className="mr-1 h-3 w-3" />{t}</Badge>)}
                    </div>
                  </div>
                ) : null}
              </div>
              ...
              <div className="flex flex-wrap gap-3 text-sm">
                {s.github_url && <a href={s.github_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md border px-3 py-1.5 hover:bg-muted"><Github className="h-4 w-4" /> Repository</a>}
                {s.live_url && <a href={s.live_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md border px-3 py-1.5 hover:bg-muted"><Globe className="h-4 w-4" /> Live demo</a>}
                {s.video_url && <a href={s.video_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md border px-3 py-1.5 hover:bg-muted"><Video className="h-4 w-4" /> Demo video</a>}
                {s.presentation_url && <a href={s.presentation_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md border px-3 py-1.5 hover:bg-muted"><FileText className="h-4 w-4" /> Pitch deck</a>}
              </div>
            </CardContent></Card>
            ...
            <Card><CardContent className="space-y-3 p-6">
              ...
              {membersQ.isLoading ? (
                ...
                <ul className="grid gap-2 sm:grid-cols-2">
                  {membersQ.data.map((m) => (
                    <li key={m.user_id} className="flex items-center gap-2 rounded-md border p-2 text-sm">
                      <ShieldCheck className="h-4 w-4 text-primary" />
                      <span className="truncate">{m.profile?.full_name ?? m.profile?.username ?? "—"}</span>
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-24">
              <CardContent className="space-y-4 p-6">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Weighted score</p>
                  <p className="mt-1 font-display text-3xl font-semibold text-primary">{weighted.toFixed(2)}<span className="text-base text-muted-foreground"> /100</span></p>
                  {isFinal && <Badge className="mt-2" variant="secondary">Submitted</Badge>}
                </div>
                <Separator />
                {!critQ.data?.length ? (
                  <p className="text-sm text-muted-foreground">Organizer hasn't defined criteria yet.</p>
                ) : (
                      ...
                      return (
                        <div key={c.id} className="space-y-2">
                          <div className="flex items-baseline justify-between gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">weight {c.weight} · max {c.max_score}</span>
                          </div>
                          ...
                          <Input
                            ...
                            onChange={(ev) => {
                              ...
                              setEntries((prev) => prev.map((p, i) => i === idx ? { ...p, score: v } : p));
                            }}
                          />
                          <Textarea
                            ...
                            rows={2}
                            value={e.comment}
                            onChange={(ev) => setEntries((prev) => prev.map((p, i) => i === idx ? { ...p, comment: ev.target.value } : p))}
                          />
                        </div>
                      );
                    })}
                ...
                <div className="flex flex-col gap-2">
                  <Button
                    ...
                    onClick={() => { const err = validate(); if (err) return toast.error(err); saveDraft.mutate(); }}
                  >
                    Save draft
                  </Button>
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          These scores will count toward the leaderboard. You can still edit them until the organizer freezes results.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => { const err = validate(); if (err) return toast.error(err); submit.mutate(); }}>
                          Confirm
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
