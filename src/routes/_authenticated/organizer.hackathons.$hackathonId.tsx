      ...
      <Section>
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="tracks" className="mt-6">
            <TracksTab hackathonId={hackathonId} />
          </TabsContent>
          <TabsContent value="criteria" className="mt-6">
            <CriteriaTab hackathonId={hackathonId} />
          </TabsContent>
          <TabsContent value="prizes" className="mt-6">
            <PrizesTab
              hackathonId={hackathonId}
              existing={(h.prizes as unknown as Prize[]) ?? []}
            />
          </TabsContent>
          <TabsContent value="registrations" className="mt-6">
            <RegistrationsTab hackathonId={hackathonId} title={h.title} />
          </TabsContent>
          <TabsContent value="judges" className="mt-6">
            <JudgesTab hackathonId={hackathonId} />
          </TabsContent>
          <TabsContent value="results" className="mt-6">
            <ResultsTab
              hackathonId={hackathonId}
              slug={h.slug}
              publishedAt={h.results_published_at}
            />
          </TabsContent>
          <TabsContent value="certificates" className="mt-6">
            <CertificatesTab hackathonId={hackathonId} />
          </TabsContent>
          <TabsContent value="announcements" className="mt-6">
            <AnnouncementsTab hackathonId={hackathonId} authorId={user!.id} />
          </TabsContent>
        </Tabs>
      </Section>
    </>
...
function TracksTab({ hackathonId }: { hackathonId: string }) {
  ...
  const q = useQuery({
    ...
    queryFn: () => listHackathonTracks(hackathonId),
  });
        ...
        <CardContent className="p-6">
          ...
          {q.isLoading ? (
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              No tracks yet. Add one on the right.
            </p>
          ) : (
...
function CriteriaTab({ hackathonId }: { hackathonId: string }) {
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["hackathon", hackathonId, "criteria"],
    queryFn: () => listScoringCriteria(hackathonId),
  });
  const [name, setName] = useState("");
  const [desc, setDesc] = useState("");
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {q.isLoading ? (
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              No criteria yet. Judges will need at least one.
            </p>
          ) : (
            <ul className="mt-4 divide-y divide-border">
              {(q.data ?? []).map((c) => (
                <li key={c.id} className="flex items-start justify-between gap-3 py-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Max {c.max_score} · weight {Number(c.weight).toFixed(2)}
                    </p>
                  </div>
...
function PrizesTab({ hackathonId, existing }: { hackathonId: string; existing: Prize[] }) {
  ...
  const add = () =>
    setPrizes((p) => [
      ...p,
      { id: crypto.randomUUID(), title: `Prize ${p.length + 1}`, amount: "", description: "" },
    ]);
        ...
        {prizes.length === 0 ? (
          ...
          <div className="mt-4 space-y-3">
            {prizes.map((p, i) => (
              <div
                key={p.id}
                className="grid gap-3 rounded-lg border border-border p-4 sm:grid-cols-[1fr_180px_2fr_auto]"
              >
                <div className="grid gap-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setPrizes((x) =>
                        x.map((y, j) => (j === i ? { ...y, title: e.target.value } : y)),
                      )
                    }
                  />
                </div>
                <div className="grid gap-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setPrizes((x) =>
                        x.map((y, j) => (j === i ? { ...y, amount: e.target.value } : y)),
                      )
                    }
                ...
                <div className="grid gap-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setPrizes((x) =>
                        x.map((y, j) => (j === i ? { ...y, description: e.target.value } : y)),
                      )
                    }
                  />
                </div>
...
function RegistrationsTab({ hackathonId, title }: { hackathonId: string; title: string }) {
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["hackathon", hackathonId, "registrations"],
    queryFn: () => listRegistrations(hackathonId),
  });
  const setStatus = useMutation({
    mutationFn: ({
      ...
      id: string;
      status: "approved" | "pending" | "rejected" | "waitlist" | "withdrawn";
    }) => updateRegistrationStatus(id, status),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["hackathon", hackathonId, "registrations"] }),
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <CardContent className="p-6">
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            ...
            description="Once your hackathon is public, registered users will show up here."
          />
        ) : (
                ...
                {(q.data ?? []).map((r) => (
                  <tr key={r.id} className="border-b border-border/50">
                    ...
                    <td className="py-3 pr-2 text-right">
                      <Select
                        value={r.status}
                        onValueChange={(v) => setStatus.mutate({ id: r.id, status: v as never })}
                      >
...
function JudgesTab({ hackathonId }: { hackathonId: string }) {
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["hackathon", hackathonId, "judges"],
    queryFn: () => listJudgeAssignments(hackathonId),
  });
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {q.isLoading ? (
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              No judges yet. Add one by email on the right.
            </p>
          ) : (
            <ul className="mt-4 divide-y divide-border">
              {(q.data ?? []).map((j) => (
                <li key={j.id} className="flex items-center justify-between gap-3 py-3">
                  <div>
                    <p className="font-medium">
                      {j.profile?.full_name ?? j.profile?.username ?? "Judge"}
                    </p>
                    {j.profile?.username && (
                      <p className="text-xs text-muted-foreground">@{j.profile.username}</p>
                    )}
                  </div>
...
function AnnouncementsTab({ hackathonId, authorId }: { hackathonId: string; authorId: string }) {
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["hackathon", hackathonId, "announcements"],
    queryFn: () => listAnnouncements(hackathonId),
  });
  const [title, setTitle] = useState("");
  const [body, setBody] = useState("");
  const [audience, setAudience] = useState<"all" | "participants" | "judges" | "mentors" | "teams">(
    "all",
  );
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              if (title.trim() && body.trim()) post.mutate();
            }}
          >
            <div className="grid gap-2">
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {publishedAt
                ? `Published on ${new Date(publishedAt).toLocaleString("en-IN")}`
                : "Not published yet. Ranks and awards will be assigned automatically."}
            </p>
          </div>
          <div className="flex gap-2">
                ...
                <li key={c.id} className="flex items-center justify-between gap-3 py-3">
                  ...
                  <div className="flex items-center gap-1">
                    <Button size="sm" variant="ghost" asChild>
                      <a
                        href={`/api/public/certificates/${c.code}`}
                        target="_blank"
