...
export const Route = createFileRoute("/mentors")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Meet the operators, founders and engineers who mentor Compass Crew builders.",
      },
      { property: "og:title", content: "Mentors — Compass Crew" },
      {
        property: "og:description",
        content: "Operators, founders and engineers mentoring the crew.",
      },
    ],
    ...
    <>
      ...
      >
        <Button asChild className="bg-gradient-brand text-white hover:opacity-90">
          <Link to="/partner">Become a mentor</Link>
        </Button>
      </PageHeader>
      <Section>
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={
              <Button asChild className="bg-gradient-brand text-white hover:opacity-90">
                <Link to="/partner">Apply to mentor</Link>
              </Button>
            }
          />
        ) : (
                ...
                <CardContent className="space-y-4 p-6">
                  <div className="flex items-center gap-4">
                    {m.avatar_url ? (
                      ...
                      <span className="grid h-14 w-14 place-items-center rounded-full bg-gradient-brand font-display text-lg font-bold text-white">
                        {m.name.charAt(0)}
                      </span>
                    )}
                    <div>
                  ...
                  {m.expertise && m.expertise.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {m.expertise.slice(0, 5).map((e: string) => (
                  ...
                  <div className="flex gap-2 pt-1">
                    {safeExternalUrl(m.linkedin_url) && (
                      <a
                        href={safeExternalUrl(m.linkedin_url)}
                        target="_blank"
                    ...
                    {safeExternalUrl(m.twitter_url) && (
                      <a
                        href={safeExternalUrl(m.twitter_url)}
                        target="_blank"
                        rel="noreferrer noopener"
                        aria-label="Twitter"
                        className="text-muted-foreground hover:text-foreground"
                    ...
                    {safeExternalUrl(m.website_url) && (
                      <a
                        href={safeExternalUrl(m.website_url)}
                        target="_blank"
                        rel="noreferrer noopener"
                        aria-label="Website"
                        className="text-muted-foreground hover:text-foreground"
